...
export default function VegetarianStarters() {
  const [showForm, setShowForm] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null);
  const [orderData, setOrderData] = useState({ name: '', quantity: 1, note: '' });
const { addOrder } = useOrder();
  const items = [
    { name: 'Paneer Tikka', price: '₹180', image: '/images/starters/PaneerTikka.jpg' },
    { name: 'Hara Bhara Kabab', price: '₹160', image: '/images/starters/HaraBharaKabab.jpeg' },
    { name: 'Veg Manchurian Dry', price: '₹170', image: '/images/starters/VegManchurian.jpeg' }
  ];
  ...
  return (
    <div className="category-page">
      <h2 className="text-center mb-4">Vegetarian Starters</h2>
